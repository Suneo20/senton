...
const simple = require("./lib/krbtz.js");
const WAConnection = simple.WAConnection(_WAConnection);
const client = new WAConnection()
const { text, extendedText, contact, location, liveLocation, image, video, sticker, document, audio, product } = MessageType
const qrcode = require("qrcode-terminal")
const fs = require("fs");
const settings = JSON.parse(fs.readFileSync('./editdisini.json'))
...
async function starts() {
	    ...
	    fs.existsSync('./kirbotz.json') && client.loadAuthInfo('./kirbotz.json')
        ...
        fs.writeFileSync('./kirbotz.json', JSON.stringify(client.base64EncodedAuthInfo(), null, '\t'))
...
await client.modifyChat(Nomer, 'delete').catch(_ => _)
        ...
        require('./akira.js')(client, message, baterai )
        })
